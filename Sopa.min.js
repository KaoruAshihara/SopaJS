...
a=a[1]<=-Math.sin(Math.PI/24)?159-b/(Math.PI/16):0>b?127-b/(Math.PI/16):b==Math.PI?142:111+b/(Math.PI/16);return a};Sopa.prototype.polar=function(a,c){var b=Array(2);var d=Math.acos(c[0]*a[0]+c[1]*a[1]+c[2]*a[2]);1==Y?(d=Ba*(1-Math.cos(d)*Math.cos(c[3])),b[0]=(1+Math.cos(d))/2,b[1]=0):2==Y?(d=Math.PI/5*(1-Math.cos(d)*Math.cos(c[3])),b[0]=Math.cos(d/2)*Math.cos(d)*Math.cos(2*d)*Math.cos(4*d)*Math.cos(8*d),b[1]=0):3==Y&&(d=Math.PI/5*(1-Math.cos(d)*Math.cos(c[3])),b[0]=Math.cos(d/2)*Math.cos(d)*Math.cos(2*
d)*Math.cos(4*d)*Math.cos(8*d),b[0]=b[0]*Math.cos(16*d)*Math.cos(32*d),b[1]=63.5*d);b[0]=Math.abs(b[0]);return b};Sopa.prototype.getUnitVector=function(a,c){var b=[a[0]+c[0],a[1]+c[1],a[2]+c[2]];var d=a[0]*c[0]+a[1]*c[1]+a[2]*c[2];1<d?b.push(0):-1>d?b.push(Ba):b.push(Math.acos(d)/2);d=Math.sqrt(b[0]*b[0]+b[1]*b[1]+b[2]*b[2]);0<d?(b[0]/=d,b[1]/=d,b[2]/=d):(b[0]=0,b[1]=1,b[2]=0);return b}};